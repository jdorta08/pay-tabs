...
function Card({ status, setStatus, data }) {
  ...
  return (
    <div className="w-full flex flex-col">
      <div key={data.id} className="w-8/12 bg-gray-100 mx-7 my-2 drop-shadow-md p-8 -md:w-11/12 -md:mx-auto">
        <div className="flex flex-col">
          ...
          <div className="flex justify-between pt-4">
            <button className="bg-blue-600 text-white px-4 py-2 rounded-sm " onClick={statusHandler}>
              Mark as {buttonText}
            </button>
            <button className=" text-gray-500 px-4 py-2 rounded-sm">Archive Tasks</button>
          </div>
        </div>
      </div>
    </div>
